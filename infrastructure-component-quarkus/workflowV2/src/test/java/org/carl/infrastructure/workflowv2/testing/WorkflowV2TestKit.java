package org.carl.infrastructure.workflowv2.testing;
 ...
 * reuse it.)
 */
public final class WorkflowV2TestKit implements AutoCloseable {
    ...
    @SafeVarargs
    public static WorkflowV2TestKit inMemory(String taskQueue, ProcessDefinition<?, ?, ?>... defs) {
        registerAll(defs);
        TestWorkflowEnvironment env = TestWorkflowEnvironment.newInstance();
        registerWorker(env.newWorker(taskQueue));
        env.start();
        return new WorkflowV2TestKit(taskQueue, env.getWorkflowClient(), env, null, null);
    }

    /** A real Temporal server (e.g. host:port). No time-skipping — avoid long awaits. */
    @SafeVarargs
    public static WorkflowV2TestKit remote(
        ...
        factory.start();
        return new WorkflowV2TestKit(taskQueue, client, null, factory, stubs);
    }
    ...
    public <S> S awaitResult(String processId, String bizId, Class<S> stateType) {
        try {
            ...
            throw new WorkflowV2ExecutionException(
