...
// build.gradle.kts
dependencies {
    implementation(project(":infra-component-qdrant-grpc"))
}
```
...
| 类 / 接口                   | 包                                     | 说明                                                                                 |
|---------------------------|---------------------------------------|------------------------------------------------------------------------------------|
| `QdrantGrpcClient`        | `org.carl.infra.qdrant`      | 核心客户端，持有 Vert.x gRPC 连接，暴露 `getCollectionsGrpcClient()` / `getPointsGrpcClient()` |
| `QdrantGrpcClientProvider`| `org.carl.infra.qdrant`      | CDI `@Produces` bean，读配置自动构造 `QdrantGrpcClient`，Quarkus 环境无需手动实例化               |
| `IQdrantAbility`          | `org.carl.infra.qdrant`      | 可选接口，业务 bean 实现后直接调 `getPoints()` / `getCollections()`，省去注入步骤                   |
...
import org.carl.infra.qdrant.QdrantGrpcClient;
import org.carl.infra.qdrant.factory.*;
