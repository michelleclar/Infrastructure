...
`infra-component-persistence-jooq` 是 jOOQ 持久化 core，提供 `PersistenceContext`、能力接口、DSLContext 工厂、schema metadata 读取和 table builder。Quarkus persistence adapter 应依赖该模块完成数据访问能力装配。
...
- Quarkus datasource 到 `DSLContext` 的装配属于 `infra-component-quarkus:persistence`。
...
**当前依赖了什么**：`api(libs.bundles.persistence.jooq)`、`implementation(project(":infra-component-log"))`，测试 runtime 使用 PostgreSQL driver。
