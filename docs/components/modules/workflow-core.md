...
`infra-component-workflow-core` 是纯 Java 工作流引擎核心库，包含流程定义、Flow DSL、NodeHandler SPI、内置节点 handler、图结构校验、路由、节点配置编解码和 interceptor SPI。它没有 Temporal 依赖。
...
- 只定义流程、校验流程和扩展节点类型；实际运行在 Temporal 上时使用 `infra-component-workflow-temporal`。
...
- `./gradlew :infra-component-workflow-core:test` 通过。
...
**如何使用**：依赖 `infra-component-workflow-core`，使用 `Flow.define(...)` 声明流程，使用 `BuiltInNodes` 创建节点，用 `GraphValidator` 校验，再交给 runtime 或持久化保存。

**当前依赖了什么**：`implementation(project(":infra-component-log"))`、Jackson BOM 和 `jackson-databind`、`jakarta.el-api:5.0.1`、`org.glassfish:jakarta.el:5.0.0-M1`。
