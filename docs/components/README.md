...
本文档覆盖 `infra-component-quarkus/` 之外的顶层 `infra-component-*` 组件。与 [Quarkus 适配层解耦说明](../quarkus-decoupling/README.md) 配套使用：这里描述可独立复用的 core/API/implementation 组件，Quarkus 目录只负责框架适配。

## 组件清单

| 组件 | 构建状态 | 定位 |
|------|----------|------|
| `infra-component-dto` | 已 include | Command/Query/Response/PageQuery 基类 |
| `infra-component-log` | 已 include | 统一日志接口与 SLF4J/JBoss 适配 |
| `infra-component-utils` | 已 include | 字符串、集合、脱敏、DAG 等工具 |
| `infra-component-http` | 已 include | 独立 HTTP client 抽象与 Apache 实现 |
...
| `infra-component-persistence-jooq` | 已 include | jOOQ 持久化上下文与元数据工具 |
| `infra-component-redis` | 已 include | Vert.x Redis 客户端封装 |
| `infra-component-rule-engine` | 已 include | 轻量规则引擎 |
| `infra-component-statemachine` | 已 include | 状态机 builder 和运行时 |
| `infra-component-qdrant-grpc` | 已 include | Qdrant gRPC client 与 request factory |
| `infra-component-embedding-grpc` | 已 include | Embedding gRPC client |

## 边界规则

- 独立组件不应依赖 `infra-component-quarkus:*`。
...
- 每个已 include 组件至少应能运行对应单模块测试：`./gradlew :infra-component-{name}:test`。
