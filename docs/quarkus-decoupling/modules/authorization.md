...
`infra-component-quarkus:authorization` 当前包含权限模型、用户身份抽象、权限定义、PDP/FGA client provider，以及直接依赖 `SecurityIdentity` 的适配代码。权限领域模型和判断规则不应绑定 Quarkus。
...
- core 可以依赖 `infra-component-dto`、`infra-component-log`、`infra-component-utils`。
- 若保留 PDP 客户端，建议放到单独实现包或 `infra-component-authorization-pdp`，避免 core 接口层被具体服务污染。
- adapter 可以依赖 `io.quarkus.security.identity.SecurityIdentity`，core 不得依赖。

## 拆分任务清单

- 创建 `infra-component-authorization`。
...
- `./gradlew :infra-component-authorization:test` 通过。
- `./gradlew :infra-component-quarkus:authorization:test` 通过。
...
**如何使用**：Quarkus 应用依赖 `infra-component-quarkus:authorization`，通过 `AuthProvider` 或 `IUserIdentity` 获取当前用户，通过 `IPermission`、`ModulePermission`、`ResourceIPermission` 等模型表达权限。需要 FGA 时配置 `fga.api-url`、`fga.store-id`、`fga.authorization-model-id`、`fga.api-token`。
