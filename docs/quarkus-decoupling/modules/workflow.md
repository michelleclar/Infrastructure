...
`infra-component-quarkus:workflow` 当前包含 Temporal 配置、worker lifecycle、client interceptor、事务工作流构建器、workflow event/model、repository 和部分快照 DTO。它混合了 Temporal/状态机编排模型与 Quarkus 生命周期装配。
...
- `infra-component-workflow`

如果希望明确 Temporal 绑定：

- `infra-component-workflow-temporal`
...
- 与 `infra-component-statemachine` 的集成逻辑。
- Temporal client interceptor 中不依赖 Quarkus 的部分。

## 依赖边界

- workflow core 可以依赖 Temporal SDK、`infra-component-statemachine`、persistence-jooq 的接口或 repository 抽象。
...
- 复用 `infra-component-statemachine`，避免在 workflow 中复制状态机能力。
- 为工作流构建、事件生成、stamp id 和 repository 契约增加 core 测试。

## 验收标准

- `./gradlew :infra-component-workflow:test` 或 `./gradlew :infra-component-workflow-temporal:test` 通过。
- `./gradlew :infra-component-quarkus:workflow:test` 通过。
...
**如何使用**：依赖 `infra-component-quarkus:workflow`，配置 Temporal/Quarkus 相关连接和 `workflow.enable.log`。业务侧使用 `TXWorkflowBuilder`、`TXWorkflowExecuter` 或 `IStateMachineAbility` 组合状态机和 Temporal workflow。模块同时依赖 persistence 和 mq adapter。

**当前依赖**：`api(libs.quarkus.temporal)`、`api(project(":infra-component-statemachine"))`、`implementation(project(":infra-component-quarkus:persistence"))`、`implementation(project(":infra-component-quarkus:mq"))`，测试依赖 `libs.quarkus.temporal.test`。
