...
`infra-component-quarkus:web` 主要包含 JAX-RS/Vert.x Web 适配、Jackson ObjectMapper producer、异常映射、过滤器、运行时上下文和部分 HTTP client builder。多数内容天然属于 Quarkus web adapter，但通用异常模型和请求 DTO 可以考虑下沉。
...
- `infra-component-web-api`
- 或将通用异常/请求模型放入已有 `infra-component-dto`
...
- `./gradlew :infra-component-quarkus:web:test` 通过。
...
**如何使用**：依赖 `infra-component-quarkus:web`，使用 `web.*` 配置，例如 `web.super-user-id`、`web.session-timeout-hour`、`web.use-session`。业务 REST 层可复用全局异常模型、Jackson producer、运行时用户/请求上下文和相关注解。

**当前依赖**：`api(libs.bundles.web)`、`implementation(project(":infra-component-utils"))`；测试侧依赖 authorization adapter。
