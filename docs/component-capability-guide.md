...
| `infrastructure-component-utils` | 提供字符串、集合、字段反射、URL 解析、日志辅助、脱敏算法、`DAG`、`LinkedTable`。 | 直接调用 `StringUtils`、`CollectionUtils`、`FieldsUtils`、`UrlParser`；脱敏使用 `IDesensitizationAlgorithm`；拓扑或依赖排序使用 `DAG`。 | 工具类保持小而稳定；新增工具前确认是否属于更具体组件；反射工具注意 JDK 21 访问限制和性能。 |
