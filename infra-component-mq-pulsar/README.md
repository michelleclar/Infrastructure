...
`build.gradle.kts` 中声明的是 `api(project(":infra-component-mq-api"))`，因此引入本模块后 `mq-api` 的接口会自动透传到调用方 classpath。
...
本模块是 [`infra-component-mq-api`](../infra-component-mq-api) 所定义 SPI 的 Pulsar 实现。业务代码应**只 import mq-api 包下的类型**（`MQClient`、`IProducer`、`IConsumer`、`MQConfig` 等），实现类均为 `package-private`，不对外暴露。

SPI 核心接口位于：

```
org.carl.infra.mq.client.MQClient
org.carl.infra.mq.producer.IProducer<T>
org.carl.infra.mq.producer.IProducerBuilder<T>
org.carl.infra.mq.consumer.IConsumer<T>
org.carl.infra.mq.consumer.IConsumerBuilder<T>
org.carl.infra.mq.config.MQConfig
org.carl.infra.mq.model.Message<T>
org.carl.infra.mq.model.MessageBuilder<T>
```
