...
1. **系统属性** `infra.logging.framework`（值：`SLF4J` / `JBOSS_LOGGING` / `AUTO`）
...
| 方式 | 键名 | 可选值 |
|------|------|--------|
| 系统属性 | `infra.logging.framework` | `SLF4J` \| `JBOSS_LOGGING` \| `AUTO` |
...
### 包 `org.carl.infra.logging.ability`

| 类型 | 说明 |
|------|------|
| `ILogAbility` | 混入接口；实现类直接调用 `getLogger()` 即可获得与本类绑定的 `ILogger`，无需声明字段 |

### 包 `org.carl.infra.logging.adapter`（内部实现，通常不直接使用）
...
```java
import org.carl.infra.logging.ability.ILogAbility;
...
```java
import org.carl.infra.logging.LoggingConfig;
import org.carl.infra.logging.LoggingFramework;
