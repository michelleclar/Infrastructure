# infra-component-persistence-jooq
...
| 类 / 接口 | 包 | 说明 |
|---|---|---|
| `PersistenceContext` | `org.carl.infra.persistence.core` | 核心入口；封装 `DSLContext`，提供同步/异步查询方法 |
| `IPersistenceProvider` | `org.carl.infra.persistence` | 提供 `PersistenceContext dsl()` 和元数据读取 default 方法的接口，供 DAO 基类实现 |
| `IPersistenceOperations` | `org.carl.infra.persistence` | 继承 `IPersistenceProvider`，补充 `persistenceCtx()` 和 `execute(String sql)` |
| `IPersistenceAbility` | `org.carl.infra.persistence.ability` | 面向业务层的能力接口，声明 `getPersistenceOperations()` |
| `IMetadataAbility` | `org.carl.infra.persistence.ability` | 继承 `IPersistenceAbility`；提供 `getMainTable()`、`getSchema()`、`getDBInfo()` default 方法 |
| `DslContextFactory` | `org.carl.infra.persistence.engine.runtime` | 工厂方法，从 `DataSource` 构建 `DSLContext`，支持自动检测方言或手动指定 |
| `JooqCustomContext` | `org.carl.infra.persistence.engine.runtime` | 接口；`apply(Configuration)` 插入自定义 jOOQ 配置（日志、超时等），默认实现已内置 `SqlLoggerListener` |
| `ConnectionProvider` | `org.carl.infra.persistence.engine.runtime` | `Supplier<Connection>`，从 `DataSource` 获取原始连接 |
| `ConnectionCallable<T>` | `org.carl.infra.persistence.function` | `@FunctionalInterface`，`T run(Connection) throws Throwable` |
| `ConnectionRunnable` | `org.carl.infra.persistence.function` | `@FunctionalInterface`，`void run(Connection) throws Throwable` |
| `DatabaseMetadataReader` | `org.carl.infra.persistence.metadata` | 运行时读取 JDBC + PostgreSQL catalog 元数据（schema/table/column/index/check constraint） |
| `SqlLoggerListener` | `org.carl.infra.persistence.sql` | jOOQ `ExecuteListener`；在 TRACE 日志级别下打印实际 SQL 和参数（通过 logger 名 `org.carl.infra.persistence.sql` 控制） |
...
import org.carl.infra.persistence.engine.runtime.JooqCustomContext;
...
quarkus.log.category."org.carl.infra.persistence.sql".level=TRACE
```
...
./gradlew :infra-component-persistence-jooq:test
```
