package org.carl.infra.persistence.sql;
...
/** Logger (package) name is 'org.carl.infra.persistence.sql' */
public class SqlLoggerListener extends LoggerListener {
    private static final ILogger logger = LoggerFactory.getLogger(SqlLoggerListener.class);

    public static org.jooq.tools.JooqLogger sqlLog =
            org.jooq.tools.JooqLogger.getLogger(SqlLoggerListener.class);

    static {
        logger.debug("sqlLog.isTraceEnabled: " + sqlLog.isTraceEnabled());
    ...
    private final Configuration abbreviateBindVariables(Configuration configuration) {
        ...
        newProviders[newProviders.length - 1] =
                new DefaultVisitListenerProvider(new BindValueAbbreviator());
    ...
    public void renderEnd(ExecuteContext ctx) {
        if (logger.isDebugEnabled()) {
            ...
            String newline =
                    Boolean.TRUE.equals(configuration.settings().isRenderFormatted()) ? "\n" : "";

            // [#2939] Prevent excessive logging of bind variables only in DEBUG mode, not
            // in TRACE mode.
            if (!logger.isTraceEnabled()) configuration = abbreviateBindVariables(configuration);
    ...
    public void executeEnd(ExecuteContext ctx) {
        if (logger.isDebugEnabled()) {
            logger.debug("Affected row(s): %d", ctx.rows());
        }
    }
            ...
            if (context.renderContext() != null) {
                ...
                if (part instanceof Param<?> param) {
                    ...
                    if (value instanceof String && ((String) value).length() > maxLength) {
                        ...
                        context.queryPart(
                                DSL.val(StringUtils.abbreviate((String) value, maxLength)));
            ...
            if (anyAbbreviations) {
                if (context.queryPartsLength() == 1) {
                    context.renderContext()
                            .sql(
                                    " -- Bind values may have been abbreviated for DEBUG logging."
                                            + " Use TRACE logging for very large bind variables.");
                }
            }
        }
    }
    ...
    static void setFinalField(
            Object reflectObj, Class<?> clsObj, String fieldName, Object newValue) {
        if (clsObj == null) {
            throw new IllegalArgumentException("(clsObj == null)");
        }
        if (fieldName == null || fieldName.isEmpty()) {
            throw new IllegalArgumentException(
                    "(fieldName == null || fieldName.length() == 0), fieldName=["
                            + fieldName
                            + "]");
        }
        try {
            ...
            String msg =
                    "reflect object class: "
                            + (reflectObj == null ? "<null>" : reflectObj.getClass().getName())
                            + ", declared class: "
                            + clsObj.getName()
                            + ", field name: "
                            + fieldName;
            logger.warn(msg + ", error: " + e);
